...
import { Trash2, Plus, Upload, X, CalendarIcon } from "lucide-react";
...
import {
  ...
  POSITION_TYPES, POSITION_STATUS, POLITICAL_LEVELS, REPRESENTATIVES, SOCIAL_ORGS,
} from "@/lib/marathi";
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        type="button"
                        variant="outline"
                        className={cn("w-full justify-start text-left font-normal", !m.dob && "text-muted-foreground")}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {m.dob ? format(new Date(m.dob), "dd/MM/yyyy") : "तारीख निवडा"}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        mode="single"
                        selected={m.dob ? new Date(m.dob) : undefined}
                        onSelect={date => {
                          const dob = date ? format(date, "yyyy-MM-dd") : "";
                          let age: number | "" = "";
                          if (dob) {
